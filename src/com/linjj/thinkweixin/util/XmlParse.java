  ...
  private String filtStr(NodeList children)
  {
    try
    {
      ...
      while(k<len)
      {
        ...
        if((k==1)&&(stemp.equals("\r\n")))continue;
        if((k==1)&&(stemp.equals("\r")))continue;
        if((k==1)&&(stemp.equals("\n")))continue;

        if((k==len)&&(stemp.equals("\r\n")))break;
        if((k==len)&&(stemp.equals("\r")))break;
        if((k==len)&&(stemp.equals("\n")))break;
  ...
  */
  ...
  private void GetXmlTree(Node node,int iBlankNum)
  {
    try
    {
      ...
      for(int j=0;j<iBlankNum;j++)sDebug.append(" ");
      ...
      if(children!=null)
      {
        ...
        for(int i=0;i<len;i++)
        {
          if((children.item(i).getFirstChild()!=null)||(children.item(i).getNodeType()==Node.ELEMENT_NODE))
          {
            if(!bAddBlank)
            {
              bAddBlank=true;
          ...
          else if((children.item(i).getNodeValue()!=null)&&(children.item(i).getNodeValue().trim().length()>0))
          {
            if(children.item(i).getNodeType()!=Root.CDATA_SECTION_NODE)
            {
              if(children.item(i).getNodeValue().trim().equals("\r\n"))continue;
              if(children.item(i).getNodeValue().trim().equals("\n"))continue;
              if(children.item(i).getNodeValue().trim().equals("\r"))continue;
            }
            if(len>1)
            {
              if(!bAddBlank)
              ...
              for(int j=0;j<iBlankNum+3;j++)sDebug.append(" ");
            }
            if(children.item(i).getNodeType()==Root.CDATA_SECTION_NODE) sDebug.append("<![CDATA[");
            if(children.item(i).getNodeType()==Root.CDATA_SECTION_NODE) sDebug.append(((CDATASection)(children.item(i))).getData());
            else sDebug.append(children.item(i).getNodeValue());
            if(children.item(i).getNodeType()==Root.CDATA_SECTION_NODE) sDebug.append("]]>");
            if(len>1) sDebug.append("\r\n");
          }
        }
      }
      if(bAddBlank)for(int j=0;j<iBlankNum;j++)sDebug.append(" ");
      sDebug.append("</"+node.getNodeName()+">\r\n");
    }
    catch (Exception ex)
    {
  ...
  public String sGetValueByName(String Name,int iNum)
  {
    try
      ...
      if((NL!=null)&&(NL.item(iNum)!=null))
      {
        ...
        else throw new Exception("Not Get Value!");
    ...
    try
    {
      ...
      if(NL!=null)
      {
        ...
        while(node==null)
        {
          ...
          iKp++;
          if(iKp==NL.getLength())break;
        }
        NodeList children=null;
        if(node!=null)children= node.getChildNodes();
      ...
      if(NL!=null)
      {
        ...
        while(node==null)
        {
          ...
          while (node!=null)
          {
            ...
            if (node!=null) System.err.println("NodeName="+node.getNodeName());
          }
